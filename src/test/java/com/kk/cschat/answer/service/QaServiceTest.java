...
class QaServiceTest {
    ...
    void setup() {
        // 본 키워드 + 답변 저장
        csQaRepository.save(new CsQa(null, "lru_test", "LRU는 가장 오래 사용되지 않은 항목을 제거하는 캐시 알고리즘입니다."));

        // 여러 alias 매핑
        keywordAliasRepository.save(new KeywordAlias(null, "엔알유테스트", "lru_test"));
        keywordAliasRepository.save(new KeywordAlias(null, "엘알유테스트", "lru_test"));
    }

    @Test
    void 기본_키워드_검색_테스트() {
        String result = qaService.getAnswer("lru_test");
        assertThat(result).contains("캐시 알고리즘");
    }

    @Test
    void 별칭으로_검색_테스트() {
        String result = qaService.getAnswer("엔알유테스트");
        assertThat(result).contains("캐시 알고리즘");
    }
